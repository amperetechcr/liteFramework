(function () {
    ...
    var RecuperacionError = {
            ...
            fetch('/api/refrescar-token', { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                .then(function (data) {
                    if (data.nuevo_token) {
                        ...
                        if (meta) {
                            meta.setAttribute('content', data.nuevo_token);
                        }
                        var inputs = document.querySelectorAll('input[name="csrf_token"]');
                        inputs.forEach(function (inp) {
                            inp.value = data.nuevo_token; });
                        if (window.actualizarTokenCSRF) {
                            window.actualizarTokenCSRF(data.nuevo_token);
                        }
                    }
        ...
        mostrarSugerencias: function (respuesta) {
            if (!respuesta.sugerencias || !respuesta.sugerencias.length) {
                return;
            }
