...
function navegarPaso(paso) {
    ...
    for (var ii = 0; ii < indicadores.length; ii++) {
        ...
        if (num === paso) el.classList.add('activo');
        else if (num < paso) el.classList.add('completado');
    }
    if (paso === 6) actualizarPreview();
}
...
function enlazarPasos() {
    ...
    for (var si = 0; si < sigs.length; si++) {
        sigs[si].addEventListener('click', function() {
            ...
            if (validarPaso(paso)) navegarPaso(paso + 1);
        });
    }
...
function agregarCampo(entidadId, nombre, tipo, requerido, unico) {
    ...
    if (tipo) div.querySelector('.campo-tipo-select').value = tipo;
...
function obtenerDatosEntidades() {
    ...
    for (var ti = 0; ti < tarjetas.length; ti++) {
        ...
        for (var fi = 0; fi < filas.length; fi++) {
            ...
            var reglas = [];
            if (campoEl.querySelector('.campo-requerido').checked) reglas.push('required');
            if (campoEl.querySelector('.campo-unico').checked) reglas.push('unique');
...
function mostrarResultado(resp) {
    ...
    if (resp.estado_operacion === true) {
        ...
        contenido.innerHTML = '<div class="alerta alinear-alerta color-exito margen-inferior-normal"><strong>Proyecto generado exitosamente</strong></div>' +
            ...
            (d.pasos_siguientes || []).map(function(p) { return '<li>' + escapar(p) + '</li>'; }).join('') +
            '</ol></div>';
    } else {
...
function init() {
    ...
    if (btnAgregarEntidad) {
        btnAgregarEntidad.addEventListener('click', function() { agregarEntidad('', null); });
    ...
    if (form) {
        form.addEventListener('submit', function(e) { e.preventDefault(); enviarGeneracion(); });
    }
    ...
    for (var ii = 0; ii < inputsForm.length; ii++) {
        inputsForm[ii].addEventListener('change', function() { if (pasoActual === 6) actualizarPreview(); });
        inputsForm[ii].addEventListener('input', function() { if (pasoActual === 6) actualizarPreview(); });
    }
}
