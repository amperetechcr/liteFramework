...
**Agregado:** `lite_ping` — tool health check del HTTP MCP server (retorna PID, sin parámetros)

**Corregido:** `mcp_server.py` — ahora mata proceso huérfano en puerto 5003 antes de iniciar `lite_mcp_http.py`, y verifica salud vía `lite_ping` con hasta 10 reintentos antes de rendirse. Anteriormente, si un proceso previo quedaba vivo en el puerto, el nuevo HTTP server fallaba silenciosamente y CrewAI perdía `lite_call`.

**Documentación:** TOOLS.md actualizado: 34 tools (6 mecánicas + 25 CLI + 3 tácticas), sección `lite_ping` agregada, problema conocido del HTTP server marcado como resuelto.
